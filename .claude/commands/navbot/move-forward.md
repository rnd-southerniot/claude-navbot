...
Then run (a background reader thread drains + checksum-validates the serial so `STATE`/stall detection is reliable; main thread streams `CMD_VEL` at 10 Hz, then STOP):
...
def reader():
    while run:
        ...
        try:
            if len(cs)<2 or xorcsum(body)!=int(cs[:2],16): continue   # drop partial/corrupt
...
print("states:", st)
print("RESULT:", "FORWARD OK" if dL>0 and dR>0 and 'FAULT STALL' not in st else "review")
s.close()
PY
EOF
```
