---
description: Test LEFT motor BACKWARD (TEST_PWM, PID-bypassed) and verify encoder counts down
allowed-tools: Bash(ssh navbot-pi:*)
---

Test the **LEFT motor driving BACKWARD** on the navbot (RP2040 fw 1.3.0, motor M2).
...
Then run (LEFT at −30% duty for 3 s, TEST_PWM auto-stops):
...
Interpret: PASS = LEFT delta clearly negative (< ~−50), RIGHT ~0, no `FAULT STALL`. Report plainly; leave motors stopped.
